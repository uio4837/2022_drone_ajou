...
R과 G의 차이가 >20일때, R과 B의 차이가 >10일때, G와 B의 차이가 < 29 일때 입니다. 

if에서 OR 구문을 사용하여 이 범위 내에 들면 드론이 촬영한 이미지의 각 3가지 영역을 모두 0처리(검은색)합니다. 즉 초록색을 식별하지 못한 것입니다.

else 구문에서는 임계값들의 범위 내에 모두 들지 않으면 추출하고자 하는 초록색을 G=255로 초록색으로 나타내고 이를 제외한 나머지 색들을 R=0, B=0 으로 설정하여 같이 검은색 처리합니다.
